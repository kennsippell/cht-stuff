select  
  ...
  percentile_disc(0.5) within group (order by total_time asc) as percentile_50th,
  percentile_disc(0.9) within group (order by total_time asc) as percentile_90th,
  percentile_disc(0.99) within group (order by total_time asc) as percentile_99th,
  sum(total_time) as sum_duration
from medic_api_log
group by 1
order by 6 desc
;
