select
  ...
  COUNT(*) filter (where url_bucket = '/medic/_changes') as changes_request_count,
...
select
  date_trunc('hour', created) 
    + make_interval(0,0,0,0,0, extract(minute FROM created)::int / 10 * 10) AS ten_min_bucket,
  percentile_cont(0.5) within group (order by queued asc) as haproxy_queue_percentile_50th,
  percentile_cont(0.90) within group (order by queued asc) as haproxy_queue_percentile_90th,
  percentile_cont(0.99) within group (order by queued asc) as haproxy_queue_percentile_99th
from haproxy_queue
where created is not null
group by 1
order by 1
